...
function normalizeQueryParams(
    ...
    return {
        ...
        pageSize: Math.min(
            50,
            Number(params?.pageSize ?? DEFAULT_QUERY.pageSize) ||
...
export async function getCustomerCoupons(
    ...
    try {
        ...
        type RawCoupon = {
            coupon_id: string;
            coupon_code: string;
            coupon_status: CustomerCoupon["coupon_status"];
            coupon_expires_at: string | null;
            coupon_redeemed_at: string | null;
            order_items: {
                offers: RawOffer | RawOffer[] | null;
            }[];
        };

        const coupons: CustomerCoupon[] = ((data ?? []) as RawCoupon[]).map((row) => {
    const orderItem = row.order_items?.[0] ?? null;

    const rawOffer = orderItem?.offers ?? null;
    const offer = Array.isArray(rawOffer) ? (rawOffer[0] ?? null) : rawOffer;

    const companies = offer?.companies ?? null;
    ...
    return {
        ...
        company_name: company?.company_name ?? "Empresa no disponible",
    };
});
