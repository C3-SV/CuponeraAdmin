    ...
    async function loadCustomersAndCoupons() {
      ...
      try {
        ...
        const customerProfileMap = new Map(
          customerProfiles.map((customerProfile) => [customerProfile.user_id, customerProfile]),
        );
        ...
        const userIds = new Set<string>([
          ...customerProfiles.map((customerProfile) => customerProfile.user_id),
        ...
        if (userIdsList.length > 0) {
          const profilesResult = await supabase
            .from("profiles")
            .select("user_id, first_names, last_names, user_role, user_is_active, created_at")
        ...
        const nextCustomers: Customer[] = userIdsList.map((userId) => {
...
=======
    return couponsByCustomer[selectedCustomer.userId] ?? [];
  }, [selectedCustomer, couponsByCustomer]);
>>>>>>> Stashed changes
  ...
  }, [customers, currentPage, PAGE_SIZE]);
          ...
          <table className="min-w-full divide-y divide-(--border)">
            ...
            <tbody className="divide-y divide-(--border) bg-(--surface)">
              ...
              {paginatedCustomers.map((customer) => (
                <tr key={customer.userId} className="hover:bg-(--surface-soft)/70">
                  <td className="px-4 py-3">
                    <p className="text-sm font-medium text-foreground">
                      {customer.nombreCompleto}
                    </p>
                    <p className="text-xs text-(--text-muted)">{customer.userId}</p>
                  </td>
                  <td className="px-4 py-3">
          ...
          <section className="relative z-10 max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-(--border) bg-(--surface) p-5 shadow-2xl lg:p-6">
            <header className="mb-4 flex items-start justify-between gap-4 border-b border-(--border) pb-4">
              <div>
                ...
                <p className="text-sm text-(--text-muted)">
                  {selectedCustomer.nombreCompleto} · {selectedCustomer.userId}
                </p>
              </div>
              <button
                type="button"
